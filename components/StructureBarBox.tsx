...
export const StructureBarBox: React.VFC<Props> = ({
  ...
  useEffect(() => {
    ...
    function assembleLayer(kontaId: KontaID): ProjectLayerItem {
      ...
      const children = konta.children.map((kontaId: KontaID) =>
        assembleLayer(kontaId)
      )
      return {
        id: kontaId,
    ...
    const layers = kontaCollection.entries.map((kontaId: KontaID) =>
      assembleLayer(kontaId)
    )
    setLayers(() => layers)
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    ...
    const renderingPoint = {
      x: clientPoint.x - containerPoint.left,
      y: clientPoint.y - containerPoint.top,
    }
    ...
    <>
      ...
      >
        {!menuOpen ? null : (
          ...
          >
            <ProjectStructureMenuBox
              onNewPlatformAdd={handleNewPlatformAdded}
            />
          </div>
        )}
